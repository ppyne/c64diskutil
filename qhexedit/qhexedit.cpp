...
//#endif
    int id = QFontDatabase::addApplicationFont(":/fonts/font_Consolas.ttf");
    QString family = QFontDatabase::applicationFontFamilies(id).at(0);
    setFont(family);

    setBackgroundColor(Qt::black);
    setLineColor(Qt::lightGray);

    setAddressAreaColor(Qt::gray);
    setAddressFontColor(Qt::black);

    setHexAreaColor(backgroundColor());
    setHexFontColor(Qt::lightGray);

    setAsciiAreaColor(backgroundColor());
    setAsciiFontColor(Qt::lightGray);

    setHighlightedAreaColor(QColor(255, 241, 95));
    setHighlightedFontColor(Qt::black);

    setSelectedAreaColor(QColor(191, 190, 255));
    setSelectedFontColor(Qt::black);

    setCursorAreaColor(Qt::green);
    setCursorFontColor(Qt::black);

    setCursorROAreaColor(Qt::red);
    setCursorROFontColor(Qt::black);
...
void QHexEdit::setBackgroundColor(const QColor &color) {
...
void QHexEdit::setHexAreaColor(const QColor &color) {
    _hexAreaColor = color;
    viewport()->update();
}

QColor QHexEdit::hexAreaColor() {
    return _hexAreaColor;
}
...
void QHexEdit::setHighlightedAreaColor(const QColor &color) {
    _highlightedAreaColor = color;
    viewport()->update();
}

QColor QHexEdit::highlightedAreaColor() {
    return _highlightedAreaColor;
}

void QHexEdit::setHighlightedFontColor(const QColor &color) {
    _highlightedFontColor = color;
    viewport()->update();
}

QColor QHexEdit::highlightedFontColor() {
    return _highlightedFontColor;
}

// Selection

void QHexEdit::setSelectedAreaColor(const QColor &color) {
    _selectedAreaColor = color;
    viewport()->update();
}

QColor QHexEdit::selectedAreaColor() {
    return _selectedAreaColor;
}

void QHexEdit::setSelectedFontColor(const QColor &color) {
    _selectedFontColor = color;
    viewport()->update();
}

QColor QHexEdit::selectedFontColor() {
    return _selectedFontColor;
}

// Ascii area

void QHexEdit::setLineColor(const QColor &color) {
    _lineColor = color;
    viewport()->update();
}

QColor QHexEdit::lineColor() {
    return _lineColor;
}
...
QColor QHexEdit::cursorAreaColor() {
    return _cursorAreaColor;
}

void QHexEdit::setCursorAreaColor(const QColor &color) {
    _cursorAreaColor = color;
    viewport()->update();
}

QColor QHexEdit::cursorFontColor() {
    return _cursorFontColor;
}

void QHexEdit::setCursorFontColor(const QColor &color) {
    _cursorFontColor = color;
    viewport()->update();
}

// Cursor read-only

QColor QHexEdit::cursorROAreaColor() {
    return _cursorROAreaColor;
}

void QHexEdit::setCursorROAreaColor(const QColor &color) {
    _cursorROAreaColor = color;
    viewport()->update();
}

QColor QHexEdit::cursorROFontColor() {
    return _cursorROFontColor;
}

void QHexEdit::setCursorROFontColor(const QColor &color) {
    _cursorROFontColor = color;
    viewport()->update();
}
...
void QHexEdit::paintEvent(QPaintEvent *event) {
    ...
    if (event->rect() != _cursorRect) {
        ...
        for (int row = 0, pxPosY = pxPosStartY; row <= _rowsShown; row++, pxPosY += _pxCharHeight) {
            ...
            for (int colIdx = 0; ((bPosLine + colIdx) < _dataShown.size() && (colIdx < _bytesPerLine)); colIdx++) {
                QColor hexbgcolor = _hexAreaColor;
                QColor hexfgcolor = _hexFontColor;
                QColor asciibgcolor = _asciiAreaColor;
                QColor asciifgcolor = _asciiFontColor;

                qint64 posBa = _bPosFirst + bPosLine + colIdx;
                if ((getSelectionBegin() <= posBa) && (getSelectionEnd() > posBa)) {
                    hexbgcolor = _selectedAreaColor;
                    hexfgcolor = _selectedFontColor;
                    asciibgcolor = _selectedAreaColor;
                    asciifgcolor = _selectedFontColor;
                } else if (_highlighting && _markedShown.at((int)(posBa - _bPosFirst))) {
                    hexbgcolor = _highlightedAreaColor;
                    hexfgcolor = _highlightedFontColor;
                    asciibgcolor = _highlightedAreaColor;
                    asciifgcolor = _highlightedFontColor;
                }

                QRect r;
                if (colIdx == 0) r.setRect(pxPosX, pxPosY - _pxCharHeight + _pxSelectionSub, 2 * _pxCharWidth, _pxCharHeight);
                else r.setRect(pxPosX - _pxCharWidth, pxPosY - _pxCharHeight + _pxSelectionSub, 3 * _pxCharWidth, _pxCharHeight);



                // Draw hex area
                painter.fillRect(r, hexbgcolor);
                hex = _hexDataShown.mid((bPosLine + colIdx) * 2, 2);
                painter.setPen(QPen(hexfgcolor));
                ...
                if (_asciiArea) {
                    ...
                    painter.setPen(QPen(asciifgcolor));
    ...
    if ((hexPositionInShowData >= 0) && (hexPositionInShowData < _hexDataShown.size())) {
        // paint cursor
        if (_readOnly) {
            painter.setPen(_cursorROFontColor);
            painter.fillRect(QRect(_pxCursorX - pxOfsX, _pxCursorY - _pxCharHeight + _pxSelectionSub, _pxCharWidth, _pxCharHeight), _cursorROAreaColor);
        } else if (_blink && hasFocus()) {
            painter.setPen(_cursorFontColor);
            painter.fillRect(_cursorRect, _cursorAreaColor);
        }
